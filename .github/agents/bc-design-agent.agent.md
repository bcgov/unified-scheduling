---
description: "BC Government Design System expert. USE FOR: choosing BC Gov colours, typography, spacing tokens, or WCAG contrast; creating new shared components aligned to BC Gov Visual Identity; reviewing components for accessibility (WCAG 2.1 AA); listing available BC Design System components; applying BC Gov design tokens to Vuetify theme mappings. DO NOT USE FOR: general Vue/TypeScript coding, API work, or non-design tasks."
...
The component library is built in **React** (`@bcgov/design-system-react-components`).
For Vue projects, design tokens are the recommended path to reimplement components consistently.
