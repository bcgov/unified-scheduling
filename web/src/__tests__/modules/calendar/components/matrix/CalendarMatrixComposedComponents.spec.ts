    ...
    const { wrapper, pinia } = await mountWithApp(MatrixView, {
      ...
      global: {
        stubs: {
          CalendarMatrixGrid: {
            props: ['model'],
            template: `<div><button class="grid-event-click" @click="$emit('eventClick', { id: 'event-1', type: '${calendarEventTypes.calendarEvent}', sourceModule: 'calendar', title: 'Event One', start: '2025-01-13T09:00:00Z' })" /><button class="grid-cell-drop" @click="$emit('cellDrop', { drag: { source: 'side-panel', itemId: 'item-1', itemType: 'user' }, drop: { resourceId: 'resource-1', date: '2025-01-13' } })" /><button class="grid-header-click" @click="$emit('headerClick', { cell: { resourceId: 'resource-1', date: '2025-01-13', groups: [] }, header: { text: 'Header', actionId: 'open-header' } })" /><button class="grid-header-action" @click="$emit('headerAction', { cell: { resourceId: 'resource-1', date: '2025-01-13', groups: [] }, header: { text: 'Header' }, actionId: 'header-action', actionType: 'button' })" /><button class="grid-event-action" @click="$emit('eventAction', { event: { id: 'event-1', type: '${calendarEventTypes.calendarEvent}', sourceModule: 'calendar', title: 'Event One', start: '2025-01-13T09:00:00Z' }, actionId: 'event-action', actionType: 'button' })" /><button class="grid-resource-add" @click="$emit('resourceAdd', { id: 'resource-1', type: 'room', title: 'Room 101', action: { actionId: 'add-resource', ariaLabel: 'Add resource' } })" /><button class="grid-drag-start" @click="$emit('dragStart', { source: 'side-panel', itemId: 'item-1', itemType: 'user' })" /></div>`,
          },
