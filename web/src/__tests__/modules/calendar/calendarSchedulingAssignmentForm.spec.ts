...
describe('calendarSchedulingAssignmentForm', () => {
  ...
  it('preserves an existing description when unrelated fields are edited', () => {
    const payload = buildCreateAssignmentPayload({
      formData: {
        ...baseFormData,
        description: 'Courtroom coverage',
        notes: 'Updated notes',
      },
      timeZoneId: 'America/Vancouver',
      locationId: 12,
  ...
  it('preserves per-link assigned users from assignment entry responses', () => {
    expect(
      resolveShiftEntryLinksFromAssignmentEntry({
        ...
        assignedUserIds: ['feaa2a73-6898-48ae-9c32-9633b1ec5538', 'd787ac4b-7969-4509-bc2b-9c85c4cbe3cb'],
        assignmentLinks: [
          {
            id: 1,
            shiftEntryId: 200,
  ...
  it('sends complete assignment entry relationships in the aggregate request', () => {
  ...
  it('omits assignment entry relationships when an edit leaves them untouched', () => {
    const payload = buildCreateAssignmentPayload({
      formData: baseFormData,
      timeZoneId: 'America/Vancouver',
      locationId: 12,
      assignmentOptions: [{ code: 7, description: 'Court assignment' }],
      includeShiftEntryLinks: false,
    });

    expect(payload?.kind).toBe('entry');
    expect(payload?.body).not.toHaveProperty('shiftEntryLinks');
  });

  it('sends an empty relationship collection when an edit removes every linked shift', () => {
    const payload = buildCreateAssignmentPayload({
      formData: { ...baseFormData, shiftEntryLinks: [] },
      timeZoneId: 'America/Vancouver',
      locationId: 12,
      assignmentOptions: [{ code: 7, description: 'Court assignment' }],
      includeShiftEntryLinks: true,
    });

    expect(payload?.body).toMatchObject({ shiftEntryLinks: [] });
  });

  it('omits assignment series relationships when an edit leaves them untouched', () => {
    const payload = buildCreateAssignmentPayload({
      formData: {
        ...baseFormData,
        repeatMode: 'custom',
        recurrenceRule: 'RRULE:FREQ=WEEKLY;COUNT=4',
        shiftSeriesLinks: [{ shiftSeriesId: 200, assignedUserIds: ['feaa2a73-6898-48ae-9c32-9633b1ec5538'] }],
      },
      timeZoneId: 'America/Vancouver',
      locationId: 12,
      assignmentOptions: [{ code: 7, description: 'Court assignment' }],
      includeShiftSeriesLinks: false,
    });

    expect(payload?.kind).toBe('series');
    expect(payload?.body).not.toHaveProperty('shiftSeriesLinks');
  });

  it('sends an empty series relationship collection when every linked shift series is removed', () => {
    const payload = buildCreateAssignmentPayload({
      formData: {
        ...baseFormData,
        repeatMode: 'custom',
      ...
      locationId: 12,
      assignmentOptions: [{ code: 7, description: 'Court assignment' }],
      includeShiftSeriesLinks: true,
    });

    expect(payload?.body).toMatchObject({ shiftSeriesLinks: [] });
  });
});
