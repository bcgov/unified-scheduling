...
describe('calendarSchedulingAssignmentsContribution', () => {
  ...
  it('fails closed and does not fetch assignments or definitions without an active location', async () => {
    ...
    const { calendarSchedulingAssignmentsContribution } =
      await import('@/modules/scheduling/contributions/calendarSchedulingAssignmentsContribution');
  ...
  it('queries through the exclusive Calendar end boundary without adding a day', async () => {
    const getApiSchedulingAssignmentsEntries = vi.fn().mockReturnValue(createFetchResult([]));
    vi.doMock('@/api-access/generated/assignment/assignment', () => ({ getApiSchedulingAssignmentsEntries }));
    vi.doMock('@/api-access/generated/assignment-definition/assignment-definition', () => ({
      getApiSchedulingAssignmentDefinitions: vi.fn().mockReturnValue(createFetchResult([])),
    }));

    const { calendarSchedulingAssignmentsContribution } =
      await import('@/modules/scheduling/contributions/calendarSchedulingAssignmentsContribution');
    await calendarSchedulingAssignmentsContribution.load({
    ...
    expect(getApiSchedulingAssignmentsEntries).toHaveBeenCalledWith(
      expect.objectContaining({
        StartAtUtc: '2026-03-01T08:00:00.000Z',
        EndAtUtc: '2026-03-09T07:00:00.000Z',
      }),
      expect.anything(),
    );
  });
