...
describe('calendarAlertStore', () => {
  ...
  it('clears alerts from a specific source', () => {
    const store = useCalendarAlertStore();

    store.setAlert({ id: 'calendar.location.required', severity: 'warning', message: 'Please select a location.', source: 'calendar' });
    store.setAlert({ id: 'scheduling.load.failed', severity: 'error', message: 'Unable to load scheduling data.', source: 'scheduling' });
    store.setAlert({ id: 'training.load.failed', severity: 'error', message: 'Unable to load training data.', source: 'training' });
    store.clearAlertsBySource('scheduling');

    expect(store.alerts.map((alert) => alert.id)).toEqual(['calendar.location.required', 'training.load.failed']);
  });
});
