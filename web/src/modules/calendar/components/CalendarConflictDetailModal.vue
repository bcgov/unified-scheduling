...
const conflictSection = computed(() => {
  const sides = resolveCalendarConflictSides(props.conflict, props.currentEventId);
  return {
    entry: sides.current,
    overlaps: sides.other,
  };
});
...
<template>
  <UaModal title="Conflict summary" tone="error" width="720" :loading="loading" @close="emit('close')">
    ...
    <section class="calendar-conflict-detail" aria-label="Calendar conflict">
      <p class="calendar-conflict-detail__time-zone">Times shown in {{ timeZone }}</p>
