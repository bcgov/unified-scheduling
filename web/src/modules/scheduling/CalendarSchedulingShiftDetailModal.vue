...
import { syncAssignmentEntryLinks, type ExistingShiftEntryLink } from './calendarSchedulingShiftAssignmentApi';
...
async function handleSaveEdit() {
  ...
  try {
    ...
    if (entityWasSaved) {
      ...
      apiError.value = `The shift was updated, but its relationships could not be fully updated: ${reason}`;
    } else {
...
async function syncEditedShiftEntryAssignmentLinks(formData: ShiftResourceFormData) {
  ...
  for (const [assignmentEntryId, assignedUserIds] of desiredLinksByAssignmentEntryId) {
    ...
    await syncAssignmentEntryLinks(
      ...
      toExistingShiftEntryLinks(existingLinksForAssignment),
    );
  }
  ...
  for (const [assignmentEntryId, existingLinksForAssignment] of existingLinksByAssignmentEntryId) {
    ...
    await syncAssignmentEntryLinks(assignmentEntryId, [], toExistingShiftEntryLinks(existingLinksForAssignment));
  }
}

function toExistingShiftEntryLinks(
  links: NonNullable<ShiftEntryResponse['assignmentLinks']>,
): ExistingShiftEntryLink[] {
  return links.flatMap((link) =>
    typeof link.id === 'number' && typeof link.shiftEntryId === 'number'
      ? [
          {
            id: link.id,
            shiftEntryId: link.shiftEntryId,
            assignedUserIds: link.userIds ?? [],
          },
        ]
      : [],
  );
