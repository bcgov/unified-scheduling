...
function formatShiftTimeRange(shift: CalendarAssignmentPartialCoverageShift) {
  const timeZone = shift.timeZoneId ?? Intl.DateTimeFormat().resolvedOptions().timeZone;
  const start = shift.start ? formatCalendarTime(shift.start, timeZone) : '';
  const end = shift.end ? formatCalendarTime(shift.end, timeZone) : '';
