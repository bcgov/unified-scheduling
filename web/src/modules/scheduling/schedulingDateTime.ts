...
export function toUtcCalendarDayRange(date?: string, timeZone?: string) {
  if (!date) {
    return null;
  }
  ...
  const startAtUtc = start.toUTC().toISO({ suppressMilliseconds: true });
  const endAtUtc = start.plus({ days: 1 }).toUTC().toISO({ suppressMilliseconds: true });
  return startAtUtc && endAtUtc ? { startAtUtc, endAtUtc } : null;
}
