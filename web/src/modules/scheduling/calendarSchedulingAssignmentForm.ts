...
type AssignmentRequestFormFields = Omit<
  ...
  'color' | 'locationId' | 'shiftEntryLinks'
>;
...
export interface BuildCreateAssignmentPayloadOptions {
  ...
  includeShiftEntryLinks?: boolean;
  includeShiftSeriesLinks?: boolean;
}
...
export function buildCreateAssignmentPayload(
  ...
  if (options.formData.repeatMode === 'custom' && options.formData.recurrenceRule) {
    const shiftSeriesLinks =
      options.includeShiftSeriesLinks === false
        ? undefined
        : options.formData.shiftSeriesLinks?.map((link) => ({
    ...
    const result = assignmentSeriesRequestSchema.safeParse({
      ...
      ...(shiftSeriesLinks === undefined ? {} : { shiftSeriesLinks }),
    });
    if (!result.success) {
      return null;
    }
    ...
    return { kind: 'series', body };
  }

  // Undefined preserves existing relationships; [] intentionally removes all relationships.
  const shiftEntryLinks =
    options.includeShiftEntryLinks === false
      ? undefined
      : options.formData.shiftEntryLinks?.map((link) => ({
  ...
  const result = assignmentEntryRequestSchema.safeParse({
    ...
    ...(shiftEntryLinks === undefined ? {} : { shiftEntryLinks }),
  ...
  const { shiftEntryIds: _shiftEntryIds, ...body } = result.data;
