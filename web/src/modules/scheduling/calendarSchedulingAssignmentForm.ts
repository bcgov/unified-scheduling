...
export type AssignmentFormData = Partial<zod.infer<typeof PostApiSchedulingAssignmentsEntriesBody>> & {
...
export type AssignmentSavePayload =
  | { kind: 'entry'; body: AssignmentEntryRequest }
  | { kind: 'series'; body: AssignmentSeriesRequest };
...
const assignmentEntryRequestSchema = PostApiSchedulingAssignmentsEntriesBody.extend({
  ...
  assignmentCategoryTypeId: zod.number().min(1),
  assignmentSubCategoryTypeId: zod.number().min(1),
  capacity: zod.number().min(1),
  shiftEntryIds: zod.array(zod.number()).optional(),
  shiftEntryLinks: zod
    .array(zod.object({ shiftEntryId: zod.number().min(1), assignedUserIds: zod.array(zod.string().uuid()).min(1) }))
    .optional(),
});
...
const assignmentSeriesRequestSchema = PostApiSchedulingAssignmentsSeriesBody.extend({
  ...
  shiftSeriesLinks: zod
    .array(zod.object({ shiftSeriesId: zod.number().min(1), assignedUserIds: zod.array(zod.string().uuid()).min(1) }))
    .optional(),
});
...
export function resolveShiftEntryLinksFromAssignmentEntry(entry: AssignmentEntryResponse): ShiftEntryLinkFormData[] {
  ...
  if (assignmentLinks.length) {
    return assignmentLinks.flatMap((link) => {
      const shiftEntryId = Number(link.shiftEntryId);
      if (!Number.isInteger(shiftEntryId) || shiftEntryId <= 0) {
        return [];
      }
...
export function buildCreateAssignmentPayload(
  ...
  const assignmentCategoryTypeId = options.formData.assignmentCategoryTypeId;
  const assignmentSubCategoryTypeId = options.formData.assignmentSubCategoryTypeId;
  ...
  if (
    ...
    !assignmentCategoryTypeId ||
    !assignmentSubCategoryTypeId
  ) {
    return null;
  }
  ...
  if (options.formData.repeatMode === 'custom' && options.formData.recurrenceRule) {
    ...
    return result.success ? { kind: 'series', body: result.data } : null;
  }
  ...
  return result.success ? { kind: 'entry', body: result.data } : null;
}
