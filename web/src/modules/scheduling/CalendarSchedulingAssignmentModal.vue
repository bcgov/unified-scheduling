...
const assignmentMutation = useSchedulingAssignmentMutation();
const canCreateAssignment = computed(() => accessControl.hasPermission(Permissions.AssignmentsCreate));
const canEditAssignment = computed(() => accessControl.hasPermission(Permissions.AssignmentsEdit));
...
const isAssignmentDefinitionModalOpen = ref(false);
const apiError = ref('');
const assignmentLoad = useSchedulingAssignmentLoad();
const assignmentLoadError = assignmentLoad.error;
const assignmentLoadState = assignmentLoad.state;
const isLoadingAssignment = assignmentLoad.isLoading;
...
const activeLocationId = computed<number | null>(() => normalizeLocationId(formData.value.locationId));
...
const assignmentShiftOptions = useSchedulingAssignmentShiftOptions({
  locationId: activeLocationId,
  date: computed(() => formData.value.date || props.initialDate),
  timeZoneId,
  onError: (message) => {
...
const isLoadingShiftOptions = assignmentShiftOptions.isLoading;
const shiftEntries = assignmentShiftOptions.shiftEntries;
const shiftSeries = assignmentShiftOptions.shiftSeries;
...
const isReadOnly = computed(
  () =>
    ...
    (modalMode.value === 'create' && !canCreateAssignment.value) ||
    (modalMode.value === 'edit' && !canEditAssignment.value),
);
...
const visibleAssignmentDetailTabs = computed<Array<{ id: AssignmentDetailTabId; label: string }>>(() => [
  ...
  ...(assignmentLoadState.value === 'loaded' && assignmentLifecycle.value.canEdit && canEditAssignment.value
    ? [{ id: 'edit' as const, label: 'Edit' }]
    : []),
  ...(assignmentLoadState.value === 'loaded' &&
  ((assignmentLifecycle.value.canDelete && canDeleteAssignment.value) ||
    (assignmentLifecycle.value.canCancel && canExpireAssignment.value))
...
const targetDateStart = computed(() => {
  const targetDate = formData.value.date || props.initialDate;
  const parsedTargetDate = targetDate ? DateTime.fromISO(targetDate, { zone: timeZoneId.value }).startOf('day') : null;

  return parsedTargetDate?.isValid ? parsedTargetDate : DateTime.now().setZone(timeZoneId.value).startOf('day');
});
const futureShiftEntries = computed(() =>
  shiftEntries.value
    .filter((entry) => isSchedulingLinkable(entry.statusTypeCode))
    .filter((entry) => shiftEntryDaysOverlapAssignment(entry)),
);
...
const shiftSeriesOptions = computed<ShiftOption[]>(() =>
  shiftSeries.value
    ...
    .filter((series) => shiftSeriesDaysOverlapAssignment(series))
...
watch(
  ...
  (value) => {
    ...
    if (value === 'never') {
      ...
      formData.value.shiftSeriesIds = [];
      formData.value.shiftSeriesLinks = [];
      selectedShiftSeriesId.value = null;
      recurrenceError.value = '';
    } else {
      promoteShiftEntryLinksToSeriesLinks();
      formData.value.shiftEntryIds = [];
...
function promoteShiftEntryLinksToSeriesLinks() {
  const links = promoteAssignmentShiftEntryLinks(
    formData.value.shiftEntryLinks ?? [],
    formData.value.shiftSeriesLinks ?? [],
    shiftEntries.value,
    shiftSeries.value,
  );
  formData.value.shiftSeriesLinks = links;
  formData.value.shiftSeriesIds = links.map((link) => link.shiftSeriesId);
}

async function loadShiftOptions() {
  await assignmentShiftOptions.load();
}
...
async function loadUsers() {
  ...
  const loadAllUsers = Boolean(props.assignmentEntryId || props.assignmentSeriesId);
  const [locationResult, allUsersResult] = await Promise.allSettled([
    usersStore.ensureUsersForLocation(locationId),
    loadAllUsers ? usersStore.ensureAllUsers() : Promise.resolve([]),
  ]);
  ...
  users.value = locationResult.status === 'fulfilled' ? locationResult.value : [];
  allUsers.value = allUsersResult.status === 'fulfilled' ? allUsersResult.value : [];

  if (locationResult.status === 'rejected' && !isRequestAbortError(locationResult.reason)) {
    apiError.value = locationResult.reason instanceof Error ? locationResult.reason.message : 'Failed to load users.';
  }
}
...
function updateAssignmentDate(value: string) {
  ...
  const validLinks = links.filter((link) => {
    const shiftEntry = getShiftEntry(link.shiftEntryId);
    return !shiftEntry || assignmentStartsOnShiftDay(shiftEntry);
  });
  ...
  updateField('shiftEntryLinks', validLinks);
  updateField(
    'shiftEntryIds',
    validLinks.map((link) => link.shiftEntryId),
  );
}
...
function applyInitialSelections() {
  if (props.initialAssignmentDefinitionId && !hasAppliedInitialAssignmentDefinitionSelection.value) {
    ...
    updateAssignmentDefinition(props.initialAssignmentDefinitionId);
    hasAppliedInitialAssignmentDefinitionSelection.value = true;
  }
  ...
  const shiftEntryIds = hasExplicitInitialShiftEntryIds
    ? (props.initialShiftEntryIds ?? [])
    : resolveOverlappingShiftEntryIdsForCurrentAssignment();
...
function addSelectedShiftSeries() {
  ...
  updateField(
    'shiftSeriesIds',
    nextLinks.map((link) => link.shiftSeriesId),
  );
  selectedShiftSeriesId.value = null;
}
...
function addSelectedShiftEntry() {
  ...
  updateField(
    'shiftEntryIds',
    nextLinks.map((link) => link.shiftEntryId),
  );
  selectedShiftEntryId.value = null;
}
...
function openAssignmentDefinitionModal() {
  if (!isBusy.value && !isReadOnly.value && canCreateAssignment.value) {
    isAssignmentDefinitionModalOpen.value = true;
  }
}
...
function handleAssignmentDefinitionClose() {
  isAssignmentDefinitionModalOpen.value = false;
}

function handleAssignmentDefinitionSaved(assignmentDefinition: AssignmentDefinitionResponse) {
  upsertAssignmentDefinition(assignmentDefinition);
  updateAssignmentDefinition(assignmentDefinition.id);
...
function formatDateTimeRange(startAtUtc?: string | null, endAtUtc?: string | null, eventTimeZoneId?: string | null) {
  ...
  const startTime = formatCalendarTime(startAtUtc, zone);
  const endTime = end?.isValid && endAtUtc ? formatCalendarTime(endAtUtc, zone) : '';
...
function formatShiftEntryDateTimeRange(entry: ShiftEntryResponse) {
  ...
  const startTime = formatCalendarTime(entry.startAtUtc, zone);

  if (!end?.isValid) {
    return `${startDate}, ${startTime}`;
  }

  const endTime = formatCalendarTime(end.toJSDate(), zone);
...
function formatAssignmentDetailDate(value?: string) {
  ...
  return formatCalendarDateTimeDate(value, timeZoneId.value) || 'Unknown';
}

function formatLocation(locationId: unknown) {
  const parsedLocationId = normalizeLocationId(locationId);
  if (!parsedLocationId) {
...
function shiftSeriesDaysOverlapAssignment(series: ShiftSeriesResponse) {
  if (typeof series.id !== 'number') {
    return false;
  }

  const shiftEntryIds = new Set(series.shiftEntryIds ?? []);

  if (
    shiftEntries.value.some((entry) => {
      if (entry.shiftSeriesId === series.id) {
        return shiftEntryDaysOverlapAssignment(entry);
      }

      return typeof entry.id === 'number' && shiftEntryIds.has(entry.id) && shiftEntryDaysOverlapAssignment(entry);
    })
  ) {
    return true;
  }

  return shiftSeriesRecurrenceDaysOverlapAssignment(series);
}

function shiftSeriesRecurrenceDaysOverlapAssignment(series: ShiftSeriesResponse) {
  const seriesStart = parseShiftSeriesDateTime(series.startAtUtc, series.timeZoneId);
  if (!seriesStart) {
    return false;
  }

  const assignmentDayRange = getAssignmentDayRange();
  const seriesEnd = parseShiftSeriesDateTime(series.endAtUtc, series.timeZoneId) ?? seriesStart;
  const seriesDuration = seriesEnd > seriesStart ? seriesEnd.diff(seriesStart) : null;

  if (!series.recurrenceRule) {
    const seriesDayRange = getDateTimeDayRange(seriesStart, seriesEnd);
    return Boolean(seriesDayRange && dayRangesOverlap(assignmentDayRange, seriesDayRange));
  }

  try {
    const parsedRule = RRule.fromString(series.recurrenceRule);
    const rule = new RRule({
      ...parsedRule.origOptions,
      dtstart: parsedRule.origOptions.dtstart ?? seriesStart.toUTC().toJSDate(),
    });
    const searchStart = assignmentDayRange.start.minus({ days: 1 }).startOf('day').toUTC().toJSDate();
    const searchEnd = assignmentDayRange.end.plus({ days: 1 }).endOf('day').toUTC().toJSDate();
    const occurrences = rule.between(searchStart, searchEnd, true);

    return occurrences.some((occurrence) => {
      const occurrenceStart = DateTime.fromJSDate(occurrence, { zone: 'utc' }).setZone(
        series.timeZoneId || timeZoneId.value,
      );
      const occurrenceEnd = seriesDuration ? occurrenceStart.plus(seriesDuration) : occurrenceStart;
      const occurrenceDayRange = getDateTimeDayRange(occurrenceStart, occurrenceEnd);
      return Boolean(occurrenceDayRange && dayRangesOverlap(assignmentDayRange, occurrenceDayRange));
    });
  } catch {
    const seriesDayRange = getDateTimeDayRange(seriesStart, seriesEnd);
    return Boolean(seriesDayRange && dayRangesOverlap(assignmentDayRange, seriesDayRange));
...
function parseShiftSeriesDateTime(value?: string | null, eventTimeZoneId?: string | null) {
  if (!value) {
    return null;
  }

  const dateTime = DateTime.fromISO(value, { setZone: true }).setZone(eventTimeZoneId || timeZoneId.value);
...
function getShiftSeriesUserIds(shiftSeriesId: number) {
  return getShiftSeries(shiftSeriesId)?.userIds ?? [];
}

function getShiftEntryUserIds(shiftEntryId: number) {
  return getShiftEntry(shiftEntryId)?.userIds ?? [];
}
...
function parseStringArray(value: SelectValue | undefined) {
  ...
  return values.map((candidate) => String(candidate)).filter(Boolean);
}
...
function resolveOverlappingShiftEntryIdsForCurrentAssignment() {
  if (
    !props.initialAssignmentDefinitionId ||
    !formData.value.date ||
...
function getAssignmentDayRange() {
  const assignmentRange = buildLocalDateTimeRange(
    formData.value.date,
    formData.value.startTime,
    formData.value.endTime,
    timeZoneId.value,
  );
  const fallback = targetDateStart.value;

  return (
    getDateTimeDayRange(assignmentRange?.start ?? fallback, assignmentRange?.end ?? fallback) ?? {
      start: fallback,
      end: fallback,
    }
  );
...
function shiftEntryDaysOverlapAssignment(entry: ShiftEntryResponse) {
  if (!entry.startAtUtc) {
    return false;
  }
  ...
  const zone = entry.timeZoneId || timeZoneId.value;
  const start = DateTime.fromISO(entry.startAtUtc, { setZone: true }).setZone(zone);
  const end = entry.endAtUtc ? DateTime.fromISO(entry.endAtUtc, { setZone: true }).setZone(zone) : start;
  const shiftDayRange = getDateTimeDayRange(start, end);

  return Boolean(shiftDayRange && dayRangesOverlap(getAssignmentDayRange(), shiftDayRange));
}

function assignmentStartsOnShiftDay(entry: ShiftEntryResponse) {
  if (!entry.startAtUtc) {
    return false;
  }
  ...
  const zone = entry.timeZoneId || timeZoneId.value;
  const assignmentStart = assignmentRange.start.setZone(zone);
  const shiftStart = DateTime.fromISO(entry.startAtUtc, { setZone: true }).setZone(zone);
  const shiftEnd = entry.endAtUtc ? DateTime.fromISO(entry.endAtUtc, { setZone: true }).setZone(zone) : null;

  return (
    assignmentStart.hasSame(shiftStart, 'day') || Boolean(shiftEnd?.isValid && assignmentStart.hasSame(shiftEnd, 'day'))
  );
}

function getDateTimeDayRange(start: DateTime, end: DateTime) {
  if (!start.isValid || !end.isValid) {
    return null;
  }

  const adjustedEnd = end > start && end.equals(end.startOf('day')) ? end.minus({ milliseconds: 1 }) : end;

  return {
    start: start.startOf('day'),
    end: adjustedEnd.startOf('day'),
  };
}

function dayRangesOverlap(left: { start: DateTime; end: DateTime }, right: { start: DateTime; end: DateTime }) {
...
async function handleSave() {
  const hasMutationPermission = isEditMode.value ? canEditAssignment.value : canCreateAssignment.value;
  if (isReadOnly.value || !hasMutationPermission) {
    apiError.value = 'You do not have permission to save assignments.';
    return;
  }
  ...
  const payload = buildCreateAssignmentPayload({
    ...
    includeShiftEntryLinks:
      !isEditMode.value || !haveSameShiftEntryLinks(validated.shiftEntryLinks, existingShiftEntryLinks.value),
    includeShiftSeriesLinks:
      !isEditMode.value || !haveSameShiftSeriesLinks(validated.shiftSeriesLinks, existingShiftSeriesLinks.value),
  });
  if (!payload) {
  ...
  const relationshipsChanged =
    !haveSameShiftEntryLinks(validated.shiftEntryLinks, existingShiftEntryLinks.value) ||
    !haveSameShiftSeriesLinks(validated.shiftSeriesLinks, existingShiftSeriesLinks.value);
  if (relationshipsChanged && !canAssignAssignment.value) {
    apiError.value = 'You do not have permission to change assignment links.';
    return;
  }
...
function resolveAssignmentSaveError(error: unknown, fallback: string) {
  ...
  if (messages.some((message) => /^A record with the same value already exists\.?$/i.test(message.trim()))) {
    return 'An assignment of this type already exists for the selected location and date.';
  }
...
function mapAssignmentServerValidationField(fieldName: string) {
  ...
  if (normalized === 'shiftentryids') {
    return 'shiftEntryIds';
  }

  if (normalized === 'shiftseriesids') {
    return 'shiftSeriesIds';
  }
...
async function handleLifecycleAction() {
  ...
  try {
    const assignmentId = isSeriesScope.value ? props.assignmentSeriesId : props.assignmentEntryId;
    const result = assignmentId
      ? await assignmentMutation.remove(
          isSeriesScope.value ? 'series' : 'entry',
          assignmentId,
...
async function loadInitialAssignment() {
  ...
  const kind = isSeriesScope.value ? 'series' : 'entry';
  const id = kind === 'series' ? props.assignmentSeriesId : props.assignmentEntryId;
  await assignmentLoad.load(kind, id, (assignment) => {
    if (kind === 'series') {
      formData.value = createFormDataFromAssignmentSeries(assignment as AssignmentSeriesResponse);
      existingShiftSeriesLinks.value = (formData.value.shiftSeriesLinks ?? []).map((link) => ({ ...link }));
    } else {
      formData.value = createFormDataFromAssignmentEntry(assignment as AssignmentEntryResponse);
      existingShiftEntryLinks.value = (formData.value.shiftEntryLinks ?? []).map((link) => ({ ...link }));
    }
    applyScopeRestrictions();
...
function createFormDataFromAssignmentEntry(entry: AssignmentEntryResponse): AssignmentFormData {
  ...
  return {
    ...
    shiftEntryIds: shiftEntryLinks.map((link) => link.shiftEntryId),
    shiftEntryLinks,
  };
}

function haveSameShiftEntryLinks(
  left: ShiftEntryLinkFormData[] | undefined,
  right: ShiftEntryLinkFormData[] | undefined,
) {
  const normalize = (links: ShiftEntryLinkFormData[] | undefined) =>
    (links ?? [])
      .map((link) => ({
        shiftEntryId: link.shiftEntryId,
        assignedUserIds: [...link.assignedUserIds].sort(),
      }))
      .sort((first, second) => first.shiftEntryId - second.shiftEntryId);

  return JSON.stringify(normalize(left)) === JSON.stringify(normalize(right));
}

function haveSameShiftSeriesLinks(
  left: ShiftSeriesLinkFormData[] | undefined,
  right: ShiftSeriesLinkFormData[] | undefined,
) {
  const normalize = (links: ShiftSeriesLinkFormData[] | undefined) =>
    (links ?? [])
      .map((link) => ({
        shiftSeriesId: link.shiftSeriesId,
        assignedUserIds: [...link.assignedUserIds].sort(),
      }))
      .sort((first, second) => first.shiftSeriesId - second.shiftSeriesId);
...
function createFormDataFromAssignmentSeries(series: AssignmentSeriesResponse): AssignmentFormData {
  ...
  return {
    ...
    shiftSeriesIds: shiftSeriesLinks.map((link) => link.shiftSeriesId),
    shiftSeriesLinks,
  };
}
...
<template>
  <UaModal :title="modalTitle" width="840" :loading="isBusy" @close="handleClose">
    ...
    <CalendarSchedulingAssignmentDetailsPanel
    ...
    <UaFormGrid v-else-if="!isDeleteTab" label-width="150px">
      ...
      <div class="assignment-modal__field">
        <div class="assignment-modal__assignment-input">
          <UaSelect
          ...
          />
          <UaBtn
            v-if="!isReadOnly && canCreateAssignment"
            class="assignment-modal__add-type-button"
            variant="flat"
            :disabled="fieldsDisabled"
            aria-label="Add Assignment Type"
            @click="openAssignmentDefinitionModal"
          >
      ...
      <UaTextField
        ...
        :disabled="fieldsDisabled"
        @update:model-value="updateAssignmentDate"
      />
      ...
      <div v-if="shouldShowRecurringShiftLinks" class="assignment-modal__field">
        <v-select
          ...
          item-value="code"
          hide-details="auto"
          :disabled="fieldsDisabled || isLoadingShiftOptions || !canAssignAssignment"
        ...
        >
          ...
          <section class="assignment-modal__link-section">
            <div class="assignment-modal__link-section-header">
              ...
              <UaBtn
                v-if="!isReadOnly && canAssignAssignment"
                variant="text"
                :disabled="fieldsDisabled"
            ...
            <v-select
              ...
              :disabled="fieldsDisabled || !canAssignAssignment"
    ...
    <CalendarSchedulingAssignmentLifecyclePanel
      v-else
      :can-cancel="assignmentLifecycle.canCancel"
      :is-series="isSeriesScope"
    />
...
.assignment-modal__assignment-input {
  display: grid;
...
.assignment-modal__add-type-button {
  justify-self: stretch;
}

:deep(.assignment-modal__add-type-button.v-btn) {
  background: var(--ua-icon-success);
