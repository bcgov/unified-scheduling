...
import {
  ...
  type ExistingShiftEntryLink,
  type ExistingShiftSeriesLink,
} from './calendarSchedulingShiftAssignmentApi';
import {
...
const partiallySavedAssignmentId = ref<number | null>(null);
const hasPartiallySavedAssignment = computed(() => partiallySavedAssignmentId.value !== null);
...
const existingShiftEntryLinks = ref<ExistingShiftEntryLink[]>([]);
const existingShiftSeriesLinks = ref<ExistingShiftSeriesLink[]>([]);
...
const fieldsDisabled = computed(() => isBusy.value || isReadOnly.value || hasPartiallySavedAssignment.value);
...
const duplicateAssignmentError = computed(() => {
  const assignmentDefinitionId = parsePositiveInteger(formData.value.assignmentDefinitionId);
  ...
  const duplicate = (props.existingAssignmentEvents ?? []).find((event) => {
    if (activeLocationId.value && event.locationId !== activeLocationId.value) {
      return false;
    }
...
function promoteShiftEntryLinksToSeriesLinks() {
  const existingLinks = new Map((formData.value.shiftSeriesLinks ?? []).map((link) => [link.shiftSeriesId, link]));

  for (const entryLink of formData.value.shiftEntryLinks ?? []) {
    const entry = getShiftEntry(entryLink.shiftEntryId);
    const shiftSeriesId = Number(entry?.shiftSeriesId);
    if (!Number.isInteger(shiftSeriesId) || shiftSeriesId <= 0 || existingLinks.has(shiftSeriesId)) {
      continue;
    }

    const assignedUserIds = getShiftSeriesUserIds(shiftSeriesId);
    existingLinks.set(shiftSeriesId, {
      shiftSeriesId,
      assignedUserIds: assignedUserIds.length ? assignedUserIds : entryLink.assignedUserIds,
    });
  }

  const links = [...existingLinks.values()];
...
async function loadShiftOptions() {
  ...
  try {
    // The generated Shift list query types do not currently expose LocationId, so these remain broad requests.
    // The response is still filtered by the captured location below and guarded against stale responses.
    ...
    shiftSeries.value = (seriesResult.data.value ?? []).filter((series) => series.locationId === locationId);
    shiftEntries.value = (entryResult.data.value ?? []).filter((entry) => entry.locationId === locationId);
    applyInitialSelections();
  } catch (error: unknown) {
    if (shiftOptionsRequestGuard.isCurrent(requestId)) {
...
async function handleSave() {
  if (partiallySavedAssignmentId.value) {
    apiError.value = 'This assignment was already saved. Close and reopen it before retrying relationship changes.';
    return;
  }
  ...
  try {
    ...
    savedId = saveResult.data.value?.id;
    if (!savedId) {
    ...
    if (payload.kind === 'series') {
      if (isEditMode.value) {
        await syncAssignmentSeriesLinks(savedId, payload.links, existingShiftSeriesLinks.value);
      }
    } else {
      await syncAssignmentEntryLinks(savedId, payload.links, existingShiftEntryLinks.value);
    }
    ...
    if (savedId) {
      await recoverPartiallySavedAssignment(payload.kind, savedId, error);
    } else {
...
async function recoverPartiallySavedAssignment(kind: 'entry' | 'series', savedId: number, error: unknown) {
  partiallySavedAssignmentId.value = savedId;
  const reloadResult = kind === 'series' ? await loadAssignmentSeriesById(savedId) : await loadAssignmentEntry(savedId);

  if (!reloadResult.error.value && reloadResult.data.value) {
    if (kind === 'series') {
      const series = reloadResult.data.value as AssignmentSeriesResponse;
      formData.value = createFormDataFromAssignmentSeries(series);
      existingShiftSeriesLinks.value = toExistingShiftSeriesLinks(series);
    } else {
      const entry = reloadResult.data.value as AssignmentEntryResponse;
      formData.value = createFormDataFromAssignmentEntry(entry);
      existingShiftEntryLinks.value = toExistingShiftEntryLinks(entry);
    }
  }

  calendarStore.refresh();
  const reason = error instanceof Error ? error.message : 'relationships could not be fully updated';
  apiError.value = `The assignment was saved, but its relationships could not be fully updated: ${reason}`;
}
...
function toExistingShiftEntryLinks(entry: AssignmentEntryResponse): ExistingShiftEntryLink[] {
  return resolveShiftEntryLinksFromAssignmentEntry(entry).flatMap((link) =>
    typeof link.id === 'number' ? [{ ...link, id: link.id }] : [],
  );
}

function toExistingShiftSeriesLinks(series: AssignmentSeriesResponse): ExistingShiftSeriesLink[] {
  return resolveShiftSeriesLinksFromAssignmentSeries(series).flatMap((link) =>
    typeof link.id === 'number' ? [{ ...link, id: link.id }] : [],
  );
}
...
<template>
  <UaModal :title="modalTitle" width="840" :loading="isBusy" @close="handleClose">
    ...
    <template v-else-if="hasPartiallySavedAssignment" #actions>
      <UaBtn variant="outlined" :disabled="isBusy" @click="handleClose">Close</UaBtn>
    </template>
