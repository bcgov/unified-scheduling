...
export const calendarAddAssignmentAction: CalendarMatrixSidePanelAction = {
  ...
  execute: (context) => {
    const initialDate = context.model.days.find((day) => day.isToday)?.date ?? context.model.days[0]?.date;
    showCalendarSchedulingAssignmentModal(initialDate);
  },
};
...
function resolveAssignmentDefinitionId(context: { drag: { payload?: unknown } }) {
  ...
  return parsePositiveNumber((payload as { assignmentDefinitionId?: unknown }).assignmentDefinitionId);
}
...
function resolveResourceAssignmentDefinitionId(resource: unknown) {
  ...
  return parsePositiveNumber((resource as { assignmentDefinitionId?: unknown }).assignmentDefinitionId ?? fromId);
}
...
function resolveExistingAssignmentEntryId(context: CalendarMatrixDropActionContext) {
  ...
  const payloadEntryId = parsePositiveNumber(
...
function resolveExistingShiftForDroppedUser(context: CalendarMatrixDropActionContext) {
  ...
  const assignmentLocationId = parsePositiveNumber(
  ...
  return shiftEvents.find(
    (event) =>
      isShiftEvent(event) &&
      !isCancelledStatus(event.statusTypeCode) &&
...
function resolveAssignmentEntryId(event: CalendarEventBase) {
  return parsePositiveNumber((event as { metadata?: { assignmentEntryId?: unknown } }).metadata?.assignmentEntryId);
}

function resolveShiftEntryId(event: CalendarEventBase) {
  return parsePositiveNumber((event as { metadata?: { shiftEntryId?: unknown } }).metadata?.shiftEntryId);
}
