...
import { buildLocalDateTimeRange, toUtcCalendarDayRange } from './schedulingDateTime';
    ...
    try {
      ...
      if (isSeriesScope) {
        ...
        if (seriesRange) {
          ...
          assignmentSeries.value = (seriesResult.data.value ?? []).filter((series) =>
            isSchedulingLinkable(series.statusTypeCode),
          );
        }
      }
      ...
      assignmentWarning.value = buildNoTimeOverlapWarning(assignmentEntries.value, occurrences, timeZoneId);
    } catch (error: unknown) {
      if (!requestGuard.isCurrent(requestId)) {
        return;
      }
...
function buildOccurrence(
  ...
  const range = buildLocalDateTimeRange(date, startTime, endTime, timeZoneId);
  if (!range) {
    return null;
  }
...
function buildWholeDayRange(occurrences: ShiftOccurrence[], timeZoneId: string) {
  ...
  const firstDayRange = toUtcCalendarDayRange(firstDate, timeZoneId);
  const lastDayRange = toUtcCalendarDayRange(lastDate, timeZoneId);
  if (!firstDayRange || !lastDayRange) {
    return null;
  }

  return {
    startAtUtc: firstDayRange.startAtUtc,
    endAtUtc: lastDayRange.endAtUtc,
  };
}
...
function formatAssignmentEntryLabel(entry: AssignmentEntryResponse, timeZoneId: string) {
  ...
  return formatAssignmentOptionLabel(title, entry.startAtUtc, entry.endAtUtc, timeZoneId);
}
...
function formatAssignmentSeriesLabel(series: AssignmentSeriesResponse, timeZoneId: string) {
  ...
  return formatAssignmentOptionLabel(title, series.startAtUtc, series.endAtUtc, timeZoneId);
}

function formatAssignmentOptionLabel(
  title: string,
  startAtUtc: string | null | undefined,
  endAtUtc: string | null | undefined,
  timeZoneId: string,
) {
  const start = startAtUtc ? DateTime.fromISO(startAtUtc, { zone: timeZoneId }) : null;
  const end = endAtUtc ? DateTime.fromISO(endAtUtc, { zone: timeZoneId }) : null;
  if (!start?.isValid) {
    return title;
  }
