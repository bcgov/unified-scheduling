...
import { formatCalendarDateTimeDate, getTodayDateInputValue, toDateInputValue } from '@/utils/date';
...
const canCreateDefinition = computed(() => accessControl.hasPermission(Permissions.AssignmentsCreate));
const canEditDefinition = computed(() => accessControl.hasPermission(Permissions.AssignmentsEdit));
...
const isReadOnly = computed(
  () =>
    ...
    (modalMode.value === 'create' && !canCreateDefinition.value) ||
    (modalMode.value === 'edit' && !canEditDefinition.value),
);
...
function enterEditMode() {
  if (!isSaving.value && canEditDefinition.value) {
...
async function handleSave() {
  const hasPermission = isEditMode.value ? canEditDefinition.value : canCreateDefinition.value;
  if (isReadOnly.value || !hasPermission) {
    apiError.value = 'You do not have permission to save assignment definitions.';
    return;
  }
...
function formatTime(value?: string | null) {
  ...
  return formatTimeOptionValue(value);
}
...
function formatDate(value?: string | null) {
  ...
  return formatCalendarDateTimeDate(value, 'UTC') || value;
}
