...
const assignmentCategoryOptions = computed<SelectOption[]>(() =>
  assignmentCategoryTypes.value
    .filter(
      (assignmentCategory) =>
        !assignmentCategory.isArchived || assignmentCategory.id === formData.value.categoryId,
    )
...
watch(assignmentSubCategoryOptions, (options) => {
  if (
    ...
    !options.some((option) => option.code === formData.value.subCategoryId)
  ) {
  ...
  <UaModal :title="modalTitle" width="760" :loading="isSaving || isLoading" @close="handleClose">
    ...
    <UaFormGrid v-else>
      ...
      <UaSelect
        ...
        :loading="isLoading"
        @update:model-value="
          (value: SelectValue | undefined) => updateSelectField('subCategoryId', value)
        "
      />
