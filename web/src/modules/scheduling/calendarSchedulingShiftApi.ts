...
export async function loadShiftEntries() {
  const result = getApiSchedulingShiftsEntries(undefined, { options: { immediate: false } });
  await result.execute();
  return result;
}

export async function loadShiftSeriesList() {
  const result = getApiSchedulingShiftsSeries(undefined, { options: { immediate: false } });
  await result.execute();
  return result;
}
