...
) : ControllerBase
{
    ...
    [HttpGet("expiry-date")]
    [Authorize(Policy = UserTrainingsViewPolicy)]
    [ProducesResponseType(typeof(UserTrainingExpiryDateResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    public async Task<ActionResult<UserTrainingExpiryDateResponse>> CalculateExpiryDate(
        [FromQuery] UserTrainingExpiryDateRequest request,
        CancellationToken cancellationToken
    )
    {
        var result = await userTrainingService.CalculateExpiryDateAsync(request, cancellationToken);
        return Ok(result);
    }
